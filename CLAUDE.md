...
- **Hunter** — `AIFollower` (~1500 lines): sight cone + LOS, hearing, last-known-position search sweep, patrol bias toward uncollected stars, wander points, `SetThreatLevel(progress, hunting)` escalation dial. States: `Chase`, `Wander`, `Search`, `Captured`, plus `Stare` (F44: below `FloorProfile.StareCeiling`, sight triggers a bounded rush to a corner vantage point and a red-faced watch that only advances while the player looks away; it becomes `Chase` inside `StareBreakDistance` or after `StareCommitSeconds`) and `Ambush` (F45: lies in wait two cells around a corner from the star the player is heading to; ended by a direct `Pickup.OnCollectedAt` match, sight, a heard noise or a timeout). Events: `ChaseStateChanged(bool)`, `StareStateChanged(bool)`, `AmbushStateChanged(bool)`, `PlayerCaught`. Capture still happens only in `Chase`. The red is built eyes plus a face `Light` at their midpoint, both anchored off the `Head` bone (`AIPresence.BuildEyes`/`SetFaceMode`) — there is no face plate (removed F47, 21 Sep 2026). The body itself is swapped from the placeholder Timmy robot to Adam (Adam Character Pack), with locomotion clips from `Assets/SourceFiles/Animation/Hunter/` falling back to pack clips, by the editor menu item **LIGHTS OUT > Build Hunter Body** (`Assets/Editor/HunterBodyBuilder.cs`); until it is run, the robot stays. Specs: `plannings/hunter-stare-ambush-plan.md`, `plannings/hunter-body-adam-plan.md`.
