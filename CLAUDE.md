...
- **World** — `MazeGenerator` (~790 lines): seeded maze, runtime `NavMeshSurface` bake, star placement, actor placement, and the composition root described above. `UsedSeed` is the seed actually used (Inspector `seed`, a pending retry seed, or `Environment.TickCount`); it is logged and shown on the end screen. **Kit maze (F48):** when `GameFlow.UseKitMaze` is set, `BuildGeometry` takes a third branch — `BuildKitGeometry`/`BuildKitPillars` assemble the maze from the Maze Modular Puzzle Kit (`Assets/Maze/Prefabs`) instead of `FloorThemes`, reading prefab references off a `KitMazeTheme` scene object built by **LIGHTS OUT > Build Kit Maze Theme** (`Assets/Editor/KitMazeThemeBuilder.cs`). `_theme` stays `null` in this mode, so the ceiling, lockers, wall lamps, props and atmosphere all keep taking their existing primitive branches. Cell size, wall thickness and wall height are overridden to 5 m / 0.4 m / 3.9 m to fit the kit's 3/2/1 m wall pieces exactly; floors are filled by the recursive square tiler `TileFloorRect`, walls by the greedy 3/2/1 span filler `FillSpan`. Missing or incomplete theme → `LogError` and fall back to the normal maze, mirroring `ResolveTheme`. The title screen's second button, START NEW MAZE, calls `GameFlow.StartKitMaze()`; the flag lasts the whole campaign (Retry and the next floor both stay in kit mode) until `GameFlow.ReturnToMenu`. Spec: `plannings/kit-maze-plan.md`. **Maze dressing (F70):** a private `MazeDressing` view, resolved once by `ResolveDressing()` right after `ResolveKitTheme()`, is what every prop/decal/set-piece pass reads (never `_theme`/`kitTheme` directly) — themed floor → `_theme`'s dressing fields; kit mode → `kitTheme`'s common dressing with wall/ceiling prop chances forced to 0 (the kit set never gets themed wall/ceiling props); primitive fallback → all-default (empty), so an un-rebuilt scene spawns nothing new. Five new passes, each its own RNG offset from `usedSeed` so the existing carve/star/lamp/locker/prop/shard streams (+0…+5) are untouched: `BuildSetPieces` (+8, pre-bake — reserves its cell, and `_propWalls`/`_setPieceCells` bookkeeping so wall/ceiling props skip it), `BuildKitDressingPreBake` (+9, kit-only gates + trees, also pre-bake since both carry colliders), `BuildKitWallLights` (+10, kit-only, post-bake emissive wall overlays), `BuildFloorProps` (+6, post-bake, collider-less floor clutter) and `BuildDecals` (+7, post-bake, alpha-clipped analytic-placement quads — no raycasts). Built by **LIGHTS OUT > Build Floor Themes** (now `FloorThemeBuilder` split across `FloorThemeBuilder.cs` and the `partial` file `FloorThemeBuilder.Dressing.cs`, which holds the shared texture/decal/particle helpers and the common prop/set-piece/dust set) and **LIGHTS OUT > Build Kit Maze Theme** (`KitMazeThemeBuilder`, which calls `FloorThemeBuilder.BuildCommonDressing()` for its share of the same common set). Re-run both (Keep existing assets) and save the scene to get the new content; an un-rebuilt scene plays exactly as before. Spec: `plannings/maze-dressing-plan.md`.
...
- **Hunter** — `AIFollower` (~1500 lines): sight cone + LOS, hearing, last-known-position search sweep, patrol bias toward uncollected stars, wander points, `SetThreatLevel(progress, hunting)` escalation dial. States: `Chase`, `Wander`, `Search`, `Captured`, plus `Ambush` (F45: lies in wait two cells around a corner from the star the player is heading to; ended by a direct `Pickup.OnCollectedAt` match, sight, a heard noise or a timeout). Events: `ChaseStateChanged(bool)`, `AmbushStateChanged(bool)`, `PlayerCaught`. Capture still happens only in `Chase`. The red is built eyes anchored off the `Head` bone (`AIPresence.BuildEyes`/`SetFaceMode`) — there is no face plate (removed F47, 21 Sep 2026). The F44 stare/gaze state (a bounded rush to a corner vantage point and a watch that froze while looked at) was removed 22 Sep 2026 as clunky; only its ambush sibling remains. The body itself is swapped from the placeholder Timmy robot to Adam (Adam Character Pack), with locomotion clips from `Assets/SourceFiles/Animation/Hunter/` falling back to pack clips, by the editor menu item **LIGHTS OUT > Build Hunter Body** (`Assets/Editor/HunterBodyBuilder.cs`); until it is run, the robot stays. Spec: `plannings/hunter-body-adam-plan.md`.
...
- **Statics must survive a scene reload and must not survive Play mode.** `GameFlow` and `GameOutcome.IsOver` both have a `[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` reset, because with *Enter Play Mode > Reload Domain* disabled a stale `SkipMenuOnLoad` would skip the title screen on the next run. `GameFlow.IsInShop`, `PlayerWallet` and `PlayerInventory` all have the same reset, and are also cleared in `GameFlow.ReturnToMenu` — the economy lives for one campaign only. `GameFlow.UseKitMaze` (F48) follows the same rule: reset in `ResetStatics` and cleared in `ReturnToMenu`, since it is meant to last a campaign, not a Player session. Any new static run state needs the same treatment.
...
- The player is still the third-person `PlayerRobot.prefab` (two copies) with `FirstPerson = true`; the AI's body is an instance of it too, until **LIGHTS OUT > Build Hunter Body** is run to swap it for Adam (Adam Character Pack) (F47).
- `Assets/Maze/` is the imported *Maze Modular Puzzle Kit* (Poly Etereo), art only, used by the F48 "START NEW MAZE" test path (see Systems → World). `Prefabs/` has the wall/pillar/floor pieces plus doors, gaps, a switch and trees not yet wired into the generator; `Materials/PixPal_URP.mat` uses a custom Shader Graph (`Materials/Textures/ImphenziaPixPal_URP.shadergraph`) whose palette is deliberately emissive in places — some pieces glow in the dark maze, which is expected.
